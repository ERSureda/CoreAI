...
public class TariffController {
    ...
    public ResponseEntity<TariffResult> createTariff(@Valid @RequestBody CreateTariffHttpRequest request) {
        return ResponseEntity
                .status(201)
                .body(createTariffUseCase.execute(mapper.toCreateTariffCommand(request)));
    }
    ...
    public ResponseEntity<TariffResult> getTariff(@PathVariable UUID id) {
        return ResponseEntity
                .status(200)
                .body(getTariffUseCase.execute(mapper.toGetTariffCommand(id)));
    }
    ...
    public ResponseEntity<ListTariffsResult> listTariffs(@Valid ListTariffsHttpRequest request) {
        return ResponseEntity
                .status(200)
                .body(listTariffsUseCase.execute(mapper.toListTariffsCommand(request)));
    }
    ...
    public ResponseEntity<TariffResult> updateTariff(
        ...
        return ResponseEntity
                .status(200)
                .body(updateTariffUseCase.execute(mapper.toUpdateTariffCommand(id, request)));
    }
    ...
    public ResponseEntity<Void> deleteTariff(@PathVariable UUID id) {
        deleteTariffUseCase.execute(mapper.toDeleteTariffCommand(id));
        return ResponseEntity
                .status(204)
                .build();
    }
