...
public class DispatchOfferController {
    ...
    public ResponseEntity<ListOffersResult> listTripOffers(
        ...
        return ResponseEntity
                .status(200)
                .body(listTripOffersUseCase.execute(mapper.toListTripOffersCommand(tripId, request)));
    }
    ...
    public ResponseEntity<ListOffersResult> listPendingDriverOffers(@PathVariable UUID driverId) {
        return ResponseEntity
                .status(200)
                .body(listPendingDriverOffersUseCase.execute(mapper.toListPendingDriverOffersCommand(driverId)));
    }
    ...
    public ResponseEntity<OfferResult> acceptOffer(@PathVariable UUID id) {
        return ResponseEntity
                .status(200)
                .body(acceptOfferUseCase.execute(mapper.toAcceptOfferCommand(id)));
    }
    ...
    public ResponseEntity<OfferResult> rejectOffer(@PathVariable UUID id) {
        return ResponseEntity
                .status(200)
                .body(rejectOfferUseCase.execute(mapper.toRejectOfferCommand(id)));
    }
}
