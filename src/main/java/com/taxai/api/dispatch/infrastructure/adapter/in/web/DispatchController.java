...
public class DispatchController {
    ...
    public ResponseEntity<MatchTripResult> matchTrip(@PathVariable UUID tripId) {
        return ResponseEntity
                .status(200)
                .body(matchTripUseCase.execute(mapper.toMatchTripCommand(tripId)));
    }
    ...
    public ResponseEntity<OfferResult> assignNearestDriver(
        ...
        return ResponseEntity
                .status(200)
                .body(assignNearestDriverUseCase.execute(mapper.toAssignNearestDriverCommand(tripId, request)));
    }
}
